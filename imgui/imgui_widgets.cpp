...
bool ImGui::ButtonExPOS(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
bool ImGui::ButtonPOS(const char* label, const ImVec2& size_arg)
{
    return ButtonExPOS(label, size_arg, ImGuiButtonFlags_None);
}
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    // Position checkbox directly after text (close spacing)
    float checkbox_offset_x = label_size.x + 4.0f; // Very small gap after text (4px) - close to text
    
    GetWindowDrawList()->AddRectFilled(check_bb.Min + ImVec2(checkbox_offset_x, 0), check_bb.Max + ImVec2(checkbox_offset_x + 20, 7.5f), GetColorU32(it_anim->second.background), 100);
    GetWindowDrawList()->AddCircleFilled(check_bb.Min + ImVec2(checkbox_offset_x + 10 - it_anim->second.slow_circle, 9.5f), 5.5f, GetColorU32(it_anim->second.circle));
    GetWindowDrawList()->AddText(ImVec2(pos.x, check_bb.Min.y + 3.5f), GetColorU32(it_anim->second.text), label);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, float val, bool multi, ImGuiComboFlags flags)
{
    ...
    it_anim->second.text = ImLerp(it_anim->second.text, it_anim->second.opened_combo ? ImColor(255, 255, 255, 255) : ImColor(255, 255, 255, 127), g.IO.DeltaTime * 14.f);
    ...
    GetWindowDrawList()->AddRectFilled(ImVec2(bb.Min.x, bb.Min.y), ImVec2(bb.Max.x, bb.Max.y), ImColor(11, 11, 11, 255), 4.f);
    ...
    window->DrawList->AddText(ImVec2(bb.Max.x - 25, bb.Min.y + 17), ImColor(255, 255, 255, 255), "W");
    PopFont();
    ...
    window->DrawList->AddText(ImVec2(bb.Min.x + 5, bb.Min.y - 29), ImColor(255, 255, 255, 127), label);
...
bool ImGui::BeginComboPopup(ImGuiID popup_id, const ImRect& bb, ImGuiComboFlags flags)
{
    ...
    if (ImGuiWindow* popup_window = FindWindowByName(name))
        if (popup_window->WasActive)
        {
            ...
            SetNextWindowPos(pos + ImVec2(0, 5.0f));
        }

    // We don't use BeginPopupEx() solely because we have a custom name string, which we could make an argument to BeginPopupEx()
    ImGuiWindowFlags window_flags = ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_Popup | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoMove;
    PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(g.Style.FramePadding.x, 5.0f)); // Horizontally align ourselves with the framed text
...
static ImGuiTabItem* ImGui::TabBarTabListPopupButton(ImGuiTabBar* tab_bar)
{
    ...
    const float button_height = tab_bar->BarRect.GetHeight();
    if (button_height <= 0.0f)
        return NULL;

    const float button_width = button_height;
    ImRect popup_button_rect(tab_bar->BarRect.Min.x, tab_bar->BarRect.Min.y, tab_bar->BarRect.Min.x + button_width, tab_bar->BarRect.Max.y);
    tab_bar->BarRect.Min.x += button_width + g.Style.ItemInnerSpacing.x;

    const ImVec2 backup_cursor_pos = window->DC.CursorPos;
    window->DC.CursorPos = popup_button_rect.Min;

    PushID(tab_bar);
    const ImGuiID popup_id = window->GetID("TabListPopup");
    ImGuiTabItem* tab_to_select = NULL;
    if (ButtonEx("##TabListPopupButton", popup_button_rect.GetSize(), ImGuiButtonFlags_PressedOnClick | ImGuiButtonFlags_AllowItemOverlap))
        OpenPopupEx(popup_id, ImGuiPopupFlags_None);

    const ImRect button_rect = g.LastItemData.Rect;
    const ImVec2 arrow_pos = ImVec2(
        button_rect.Min.x + ImMax(0.0f, (button_rect.GetWidth() - g.FontSize) * 0.5f),
        button_rect.Min.y + ImMax(0.0f, (button_rect.GetHeight() - g.FontSize) * 0.5f)
    );
    const ImU32 arrow_col = GetColorU32(ImGuiCol_Text);
    RenderArrow(window->DrawList, arrow_pos, arrow_col, ImGuiDir_Down, 0.8f);
